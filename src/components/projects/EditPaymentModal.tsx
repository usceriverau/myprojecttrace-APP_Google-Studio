      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-emerald-50 text-emerald-700 flex items-center justify-center border border-emerald-100 shadow-xs">
              <CreditCard className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-xl font-black text-slate-900 leading-tight">Edit Customer Payment</h2>
              <p className="text-xs sm:text-sm text-slate-500 font-medium">Update amounts, dates, payer details, or replace proof records</p>
            </div>
          </div>
          <button
            ...
            className="text-slate-400 hover:text-slate-600 p-2.5 rounded-xl hover:bg-slate-100 transition-colors cursor-pointer"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSave} className="overflow-y-auto p-6 space-y-5 flex-1">
          ...
          {successMessage && (
            <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm font-semibold flex items-center gap-2.5">
          ...
          <div>
            ...
            <select
              ...
              required
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-sm sm:text-base font-semibold text-slate-900 focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-[#054AC6] focus:border-transparent transition-all"
            >
              {projects.map((p) => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="0.00"
                  className="w-full pl-9 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-lg sm:text-xl font-black text-slate-900 focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-[#054AC6] focus:border-transparent transition-all"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <Calendar className="w-4.5 h-4.5 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
                <input
                  ...
                  className="w-full pl-10 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm sm:text-base font-semibold text-slate-900 focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-[#054AC6] focus:border-transparent transition-all"
                />
              </div>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button
                ...
                className="text-xs sm:text-sm font-bold text-[#054AC6] hover:underline cursor-pointer flex items-center gap-1"
              >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                <Hash className="w-4.5 h-4.5 text-slate-400 absolute left-3.5 top-1/2 -translate-y-1/2" />
                <input
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea
                ...
                className="w-full pl-10 pr-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm sm:text-base font-medium text-slate-900 focus:bg-white focus:outline-hidden focus:ring-2 focus:ring-[#054AC6] focus:border-transparent transition-all resize-none"
              />
            </div>
          </div>
          ...
          <div className="pt-2 border-t border-slate-100">
            <div className="flex items-center justify-between mb-2">
              <label className="block text-xs sm:text-sm font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
              ...
              <button
                ...
                className="text-xs sm:text-sm font-bold text-[#054AC6] hover:text-[#03225F] flex items-center gap-1 cursor-pointer hover:underline"
              >
          ...
          <div className="pt-4 border-t border-slate-100">
            {!isConfirmingDelete ? (
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-bold text-slate-700">Delete Payment Record</p>
                  <p className="text-xs text-slate-400">Permanently delete this payment if created by mistake.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setIsConfirmingDelete(true)}
                  className="text-xs sm:text-sm font-bold text-rose-600 hover:text-rose-700 hover:bg-rose-50 px-3 py-1.5 rounded-lg border border-rose-200 transition-colors cursor-pointer flex items-center gap-1"
                >
                  <Trash2 className="w-4 h-4" />
                  Delete Payment
                </button>
              ...
              <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-xl space-y-2 animate-in fade-in duration-100">
                <p className="text-sm font-bold text-rose-900">Are you sure you want to delete this payment record?</p>
                <p className="text-xs text-rose-700">This action will remove the ${payment.amount.toLocaleString()} collection from project metrics permanently.</p>
                <div className="flex items-center gap-2 pt-1">
                  <button
                    type="button"
                    onClick={handleDelete}
                    disabled={isDeleting}
                    className="text-xs sm:text-sm font-bold bg-rose-600 hover:bg-rose-700 text-white px-3.5 py-2 rounded-lg transition-colors cursor-pointer flex items-center gap-1.5"
                  >
                  ...
                  <button
                    ...
                    disabled={isDeleting}
                    className="text-xs sm:text-sm font-semibold text-slate-600 hover:text-slate-900 px-3.5 py-2 rounded-lg hover:bg-white transition-colors cursor-pointer"
                  >
                    Cancel
                  </button>
                </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between gap-3">
          <button
            ...
            onClick={onClose}
            disabled={isSaving || isDeleting}
            className="text-xs sm:text-sm font-bold text-slate-600 hover:text-slate-900 px-4 py-2.5 rounded-xl hover:bg-slate-200/60 transition-colors cursor-pointer"
          >
            Cancel
          </button>
          ...
          <button
            ...
            onClick={handleSave}
            disabled={isSaving || isDeleting}
            className="text-xs sm:text-sm font-bold text-white bg-[#054AC6] hover:bg-[#03225F] px-5 py-2.5 rounded-xl shadow-xs transition-colors flex items-center gap-2 cursor-pointer disabled:opacity-50 min-h-[44px]"
          >
            {isSaving ? (
              <>
